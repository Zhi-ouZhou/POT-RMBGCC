...
class GBList:
    ...
    def __init__(self, data, labels, p=8):
        ...
        # 在 base.py 的 GBList 类中添加以下方法：

    def get_stats_for_tree(self):
        """
        导出给 Leading Tree 构建使用的数据 (Numpy 格式，切断梯度)
        """
        centers = self.get_centers().detach().cpu().numpy()
        # 增加 .reshape(-1) 强制转为 1D 数组，防止 K=1 时变成 0维标量
        rs = self.get_rs().detach().cpu().numpy().reshape(-1)
        # 提取每个粒球包含的样本数，同样保护一下
        counts = np.array([ball.num_smp for ball in self.granular_balls]).reshape(-1)
        return centers, rs, counts
